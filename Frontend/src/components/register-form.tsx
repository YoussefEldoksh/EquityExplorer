...
export function RegisterForm({
  ...
  const handleMetaLogin = () => {
    localStorage.setItem('oauth_provider', 'meta');
  ...
  useEffect(() => {
    ...
    if (code && !hasExchanged.current) {
      ...
      const isMeta = localStorage.getItem('oauth_provider') === 'meta';

      const exchangeCode = async () => {
        const endpoint = isMeta 
          ? `${import.meta.env.VITE_API_BASE_URL}/api/auth/login_w_meta.php`
          : `${import.meta.env.VITE_API_BASE_URL}/api/auth/login_w_github.php`;

        const providerName = isMeta ? 'Meta' : 'GitHub';
        ...
        try {
          ...
          console.error(`${providerName} exchange failed:`, error);
        }
      };
      exchangeCode();
    }
          ...
          <form className="p-6 md:p-8" onSubmit={handleSubmit}>
            <FieldGroup>
              ...
              <Field className="grid grid-cols-3 gap-4">
                <Button variant="outline" type="button" onClick={handleMetaLogin}>
