        ...
        <>
            ...
            >
                ...
                <div className='pt-20 sm:pt-30'>
                    <p className='text-xl sm:text-2xl md:text-3xl font-general-sans font-semibold px-3 sm:px-7'>
                        Popular Indices
                    </p>
                </div>
                ...
                <div className={`grid ${isMobile ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-2 sm:grid-cols-3"} px-3 sm:px-5 mt-3 sm:mt-5 gap-3 sm:gap-5`}>
                    {INDICES.map((symbol) => (
                        <div key={symbol} className="bg-[#CED1FF] px-2 pt-2 pb-4 sm:pb-9 rounded-lg">
                            <div className={`bg-[#d6d7ff] rounded-lg p-2 sm:p-3 flex ${isMobile ? "h-full" : ""}`}>
                                {loading ? (
                                    <div className="w-full flex flex-col gap-2">
                                        <Skeleton className="h-4 w-24  rounded-xl" />
                                        <Skeleton className="h-5 w-40 rounded-xl" />
                                        <Skeleton className="h-4 w-20 mt-4 rounded-xl" />
                                    </div>
                                ) : (
                ...
                <div className='px-3 sm:px-5 pt-10 sm:pt-15'>
                    <div className='mb-3'>
                        <p className='text-sm sm:text-base font-general-sans font-bold px-2 sm:px-3'>Stock Screener</p>
                        <p className='text-xl sm:text-2xl md:text-3xl font-general-sans font-bold px-2'>All Stocks</p>
                    </div>
                                ...
                                {loading
                                    ? Array.from({ length: 20 }).map((_, i) => (
                                        <TableRow key={i}>
                                            ...
                                            <TableCell className=" sm:table-cell"><Skeleton className="h-4 w-32 rounded-xl" /></TableCell>
                                            <TableCell><Skeleton className="h-4 w-16 rounded-xl" /></TableCell>
                                            <TableCell><Skeleton className="h-4 w-16 rounded-xl" /></TableCell>
                                            <TableCell className=" md:table-cell"><Skeleton className="h-4 w-20 rounded-xl" /></TableCell>
                                            <TableCell className=" lg:table-cell"><Skeleton className="h-4 w-20 rounded-xl" /></TableCell>
                                            <TableCell className=" lg:table-cell"><Skeleton className="h-4 w-12 rounded-xl" /></TableCell>
                                            <TableCell className=" md:table-cell"><Skeleton className="h-4 w-12 rounded-xl" /></TableCell>
                                            <TableCell className=" lg:table-cell"><Skeleton className="h-4 w-12 rounded-xl" /></TableCell>
                                            <TableCell className=" lg:table-cell"><Skeleton className="h-4 w-24 rounded-xl" /></TableCell>
                                        </TableRow>
                                    ))
                                    : paginatedTickers.map((ticker: any) => (
                                        <TableRow
                                            key={ticker?.symbol}
                                            className="cursor-pointer hover:bg-[#898bff] hover:text-white transition-colors duration-300"
                                            onClick={() => navigate(`/${ticker?.symbol}`)}
                                        >
                                            <TableCell>
                                                <div className='font-medium bg-[#d6d7ff] rounded-lg text-center py-2 hover:bg-black hover:text-white'>
                                                    {ticker?.symbol}
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-right  lg:table-cell">
                                                {ticker?.div ? `${ticker.div.toFixed(2)}%` : '—'}
                                            </TableCell>
